...
| `review-queue-rows-gitlab` | FORGES § 3. What GitLab and Bitbucket cannot answer | On GitLab a row's line counts stay `+0 −0` whatever the diff, because the REST API exposes no aggregate, while the file count is real once the row is expanded | e2e: GitLab rows carry no line counts |
...
| `review-batch-read-only-mode` | GUIDE § The Deck | The batch offers a read-only review that fetches the PR's own commit and checks nothing out, so several reviews share one window and none can move your working tree | e2e: a read-only batch review checks nothing out |
| `review-batch-read-only-not-a-request-mode` | GUIDE § The Deck | The read-only mode is offered by the batch only — never one of `agentFlow.reviewRequestModes` unless you add the `read-only` id yourself, so a single-row launch stays one click | unit: test/unit/engine/review/batch.test.ts |
| `review-batch-layout` | GUIDE § The Deck | Landing in a new window with several PRs asks how to lay them out; a window per PR is the plain single launch repeated | e2e: a batch review launches one worktree |
| `review-batch-layout-shared` | GUIDE § The Deck | Choosing one shared window puts every review in a single window, a session each | e2e: a shared-window batch review opens one window |
| `review-batch-threshold` | GUIDE § The Deck | Batches larger than `agentFlow.batchLaunchConfirmThreshold` confirm first and name the cost in sessions, before anything is created | e2e: a batch over the threshold names its cost in sessions |
| `review-batch-no-checkout-skip` | GUIDE § The Deck | PRs in a repo you have not checked out are named once — per repo, not per PR — and skipped; the rest launch | e2e: PRs in a repo you have not checked out are named once and skipped |
...
| `doctor-forge-mode-row` | FORGES § Bitbucket has two modes | Doctor's Bitbucket group has a mode row reading `passthrough (full)` or `projected (limited — upgrade atlassian-cli for full support)` | e2e: Doctor names passthrough mode |
| `doctor-forge-mode-projected` | FORGES § Bitbucket has two modes | A clap "unrecognized subcommand" from `bb api --help` reports the projected mode on that same row, which stays an `ok` row reading signed in | e2e: Doctor names projected mode |
...
| `orch-node-command-run` | GUIDE § The Deck | A rule pointed at a command node runs a shell command from `agentFlow.commands` (`id`, `label`, `run`, optional `detail`) or free text | e2e: a command node asks consent and act runs it |
| `orch-node-command-note` | GUIDE § The Deck | `{note}` in a command node's text is replaced with the rule's note, spliced in unquoted | unit: test/unit/engine/orchestrator/command.test.ts |
...
| `orch-node-gate-card-block` | GUIDE § The Deck | The gate's Approve and Reject also render on the CARD's Workflow block; clicking them there answers the gate, and the downstream rule fires on it | e2e: Approve on the card's own workflow block answers the gate |
...
| `orch-cond-command-succeeded` | ORCHESTRATOR_COMMANDS § You can | A rule leaving a command node offers "the command succeeded", and the second command inherits the first one's directory | e2e: command succeeded chains a second command |
...
| `orch-cond-changes-requested` | FORGES § 3. What GitLab and Bitbucket cannot answer | On GitLab (and projected Bitbucket) arming names the `changes-requested` rule as unfirable | e2e: arming a changes-requested rule on GitLab |
...
| `orch-consent` | ORCHESTRATOR_COMMANDS § One pass | Two separate consents, once each per flow: launches/seeds (naming ticket, repos and prompt mode) and shell (naming the command text); the asking pass performs nothing; approval lets the next pass act | e2e: asks consent and act runs it through /bin/sh |
| `orch-consent-disarm` | ORCHESTRATOR_COMMANDS § One pass | Disarm in the consent dialog disarms the flow and stamps no approval, so nothing runs and a later arm asks again | e2e: disarm in the consent dialog disarms the flow |
...
| `orch-retry-engine` | ORCHESTRATOR_COMMANDS § Retry, if you ask for it | A failed rule with a policy keeps its error but is not settled and is re-evaluated once its backoff passes, condition still required; a terminal failure stays settled; an "all" junction is held, not killed, by a sibling pending retry | unit: test/unit/engine/orchestrator/evaluate.test.ts |
| `orch-retry-stamps` | ORCHESTRATOR_COMMANDS § Retry, if you ask for it | `applyFired` schedules `retryAt = now + backoff` while `attempts <= max`, latches after, never retries a rule without a policy, a fail-closed edge, or a `run` without `retryOk`; a success clears the error and says `after N retries` | unit: test/unit/engine/orchestrator/runner.test.ts |
| `orch-retry-host` | ORCHESTRATOR_COMMANDS § Retry, if you ask for it | The Deck journals `retrying` right after `errored`, does not act before the backoff, acts after it without asking consent again, latches once retries are spent, and Reset clears the count and schedule but keeps the policy | unit: test/unit/deckView.test.ts |
| `orch-retry-ui` | ORCHESTRATOR_COMMANDS § Retry, if you ask for it | RETRY count/backoff controls appear on launch, seed and run rules only; a run rule adds the safe-to-re-run tick; a pending retry shows the red error with its schedule and offers Reset; a terminal failure says what it cost | ct: test/webview/OrchestratorDrawer.test.tsx |
| `orch-printed-engine` | ORCHESTRATOR_COMMANDS § Reading what a command printed | `the command printed…` fires on a host-supplied verdict keyed by the rule, only once the command's own rule has performed (succeeded or failed); an absent verdict waits; a rule off a place never fires | unit: test/unit/engine/orchestrator/evaluate.test.ts |
| `orch-printed-journal` | ORCHESTRATOR_COMMANDS § Reading what a command printed | The verdict is a case-insensitive substring match against the performer's LATEST `fired`/`errored` output; blank text, no output, or a pending performer all read false | unit: test/unit/engine/orchestrator/journal.test.ts |
| `orch-printed-host` | ORCHESTRATOR_COMMANDS § Reading what a command printed | The Deck reads the journal once per pass for flows carrying such a rule, fires on a match, waits otherwise, and posts the per-flow verdicts on `deck:flows` | unit: test/unit/deckView.test.ts |
| `orch-printed-ui` | ORCHESTRATOR_COMMANDS § Reading what a command printed | The condition is offered off a command node with a text field, marks a blank text as `no text set`, and the dry run reads the host's verdict | ct: test/webview/OrchestratorDrawer.test.tsx |
| `orch-deadline-clock` | ORCHESTRATOR_COMMANDS § Deadlines | A rule's clock starts on the first pass it is live (card on the board, command run, gate asked), only when it carries a deadline; a flow without deadlines is never written for this | unit: test/unit/engine/orchestrator/evaluate.test.ts |
| `orch-deadline-expiry` | ORCHESTRATOR_COMMANDS § Deadlines | Past the deadline with the condition unmet the rule settles as expired — a third terminal stamp, not fired and not errored, never re-evaluated until Reset; a met condition fires instead, a gone card does not stop the clock, and an "all" junction dies on an expiry | unit: test/unit/engine/orchestrator/evaluate.test.ts |
| `orch-deadline-sibling` | ORCHESTRATOR_COMMANDS § Deadlines | A sibling rule out of the same source on `a deadline here passed` fires on the pass after the expiry, and only on a sibling's expiry | unit: test/unit/engine/orchestrator/evaluate.test.ts |
| `orch-deadline-host` | ORCHESTRATOR_COMMANDS § Deadlines | The Deck stamps the clock and the expiry under the lock, journals an `expired` event, and lets the sibling fire on the next pass; re-arming restarts pending clocks; Reset clears both stamps and keeps the deadline | unit: test/unit/deckView.test.ts |
| `orch-deadline-ui` | ORCHESTRATOR_COMMANDS § Deadlines | Every rule has a WITHIN field in the inspector and the list (blank means forever, junk reverts); an expired rule reads `expired — waited Nm` in the dim voice with Reset; the card's stepper marks it `⊘` and offers Reset but never Output; the dry run reads `expires in Nm` / `would expire` | ct: test/webview/OrchestratorDrawer.test.tsx |
...
| `orch-never-auto-run-e2e` | ORCHESTRATOR_COMMANDS § Never, whatever you approved | In a real host a matching pattern outranks a flow's stored `commandConfirmedAt`: the rule latches errored naming the pattern, no consent is ever offered, and the file the command would have deleted survives | e2e: neverAutoRun outranks approval |
...
| `orch-command-output` | ORCHESTRATOR_COMMANDS § You can | stdout and stderr go to the output channel and the journal; a workflow's done/fail step offers Output, which opens the latest `fired`/`errored` line in an editor tab headed with a pointer back | e2e: a rule's output opens in an editor tab |
...
| `orch-templates-save` | ORCHESTRATOR_COMMANDS § Authoring a template directly | Save-as-template writes an envelope to `~/.agentflow/templates/`, demoting every place back to `planned` with a blank ticket and no consent stamps, and lists the row by name with its rule count | e2e: saving a flow as a template lists it by name with its rule count |
| `orch-templates-edit` | ORCHESTRATOR_COMMANDS § Authoring a template directly | Edit reopens a saved template on Canvas under its own id, with every workflow verb gone, and Save updates that same file in place rather than writing a second one | e2e: editing a saved template renames it |
| `orch-templates-delete` | ORCHESTRATOR_COMMANDS § Authoring a template directly | Delete confirms first — Cancel keeps the file, Confirm delete removes it — and never touches an instantiated workflow | e2e: deleting a template confirms first |
...
| `orch-templates-built-in-e2e` | ORCHESTRATOR_COMMANDS § Built-in starters | In a real host all three built-in rows are marked Built-in and carry no Delete, Rename or Edit control — only Duplicate | e2e: built-in starters are marked and cannot be deleted |
...
| `journal-files` | FLOW_JOURNAL § top | Every armed flow keeps `~/.agentflow/flows/<id>.log.jsonl` beside `<id>.json`, one line per event, oldest first | e2e: the journal records armed, consent and fired events in order |
...
| `journal-always-on` | FLOW_JOURNAL § top | The journal is written whenever `agentFlow.orchestrator` is on; there is no separate setting | e2e: the journal records armed, consent and fired events in order |
...
| `journal-checksum-e2e` | FLOW_JOURNAL § The fields | In a real host a corrupted line is skipped, new lines still append, and the drawer still renders | e2e: a line with a bad checksum is skipped, not fatal |
...
| `journal-event-expired` | FLOW_JOURNAL § The events | An `expired` line carries `edge`, `from`, `to`, `since`, and is not read as a run's output | unit: test/unit/engine/orchestrator/journal.test.ts |
| `journal-event-order-e2e` | FLOW_JOURNAL § The events | A real armed flow journals `armed`, `consent-asked`, `consented`, `fired` in order, each line carrying `id`, `at`, `flow` and `sum`, ids in lexical order | e2e: the journal records armed, consent and fired events in order |
...
| `journal-outlives-flow` | FLOW_JOURNAL § Lifetime | Deleting a flow removes `<id>.json` and leaves `<id>.log.jsonl` | e2e: deleting a flow leaves its journal on disk |
...
| `orch-unproven-shell` | ORCHESTRATOR_COMMANDS § Not yet proven | The command runs through Node's default shell — proven in a real editor by the child reporting `/bin/sh` in `$0`; the Windows path is still unexercised | e2e: runs it through /bin/sh |
| `orch-unproven-settings-write` | ORCHESTRATOR_COMMANDS § Not yet proven | No longer unproven: Save to settings writes the host's real `settings.json`, and the shipped example survives the append | e2e: writes agentFlow.commands into the real settings.json |
| `orch-unproven-chain` | ORCHESTRATOR_COMMANDS § Not yet proven | No longer unproven: the chained shape `place → cmd → cmd` runs end to end in a real editor, both commands appending to one file in order and the second inheriting the first's checkout | e2e: chains a second command |
| `orch-command-path-never-run-in-editor` | ORCHESTRATOR_COMMANDS § Not yet proven | No longer unproven: in a real editor the consent dialog names the command, the answer runs it in the source place's checkout, and its stdout reaches both the journal and the output channel | e2e: a command node asks consent and act runs it through /bin/sh |
...
| `forge-gitlab-queue-size` | FORGES § 3. What GitLab and Bitbucket cannot answer | GitLab's queue row size is filled on expansion with `additions`/`deletions` 0 and `changes_count` capped at `"20+"`, which renders as `20 files` | e2e: reads 20+ changes as 20 files |
| `forge-gitlab-queue-ci` | FORGES § 3. What GitLab and Bitbucket cannot answer | GitLab's queue CI chip reads `none` until the row is expanded | e2e: a GitLab row's CI reads none until expanded |
...
| `forge-bitbucket-mode-e2e` | FORGES § Bitbucket has two modes | In a real host the detection drives Doctor's mode row and the projected card shows branch CI and little else | e2e: a projected card shows branch CI and little else |
| `forge-bitbucket-no-review-queue` | FORGES § Bitbucket has two modes | `caps.reviewSearch` is `false` in both Bitbucket modes; the strip is hidden rather than shown empty | e2e: the review strip is hidden on Bitbucket in both modes |
...
| `forge-bitbucket-projected-rebase` | FORGES § Bitbucket merge is untested | Projected mode refuses `agentFlow.mergeMethod: rebase` with a message naming the setting, before any CLI call — nothing is substituted | e2e: projected mode refuses a rebase merge |
...
| `set-never-auto-run` | ORCHESTRATOR_COMMANDS § Never, whatever you approved | `agentFlow.neverAutoRun` patterns outrank every approval | e2e: outranks approval |
| `set-commands` | GUIDE § The Deck | `agentFlow.commands` entries (`id`, `label`, `run`, `detail`) feed the command picker; Save to settings appends to it | e2e: Save to settings writes agentFlow.commands into the real settings.json |
