...
| `orch-list-view-keyboard` | GUIDE § The Deck | List view builds, wires, edits and arms the same flow without a pointer | todo |
| `orch-header-workflows-button` | ORCHESTRATOR_COMMANDS § Finding them | The Workflows header button is badged `N needs you` once a workflow is waiting-on-you or stopped, else a plain count; a second click while Templates shows switches to Active rather than closing; it never mints a blank flow | todo |
| `orch-header-templates-button` | ORCHESTRATOR_COMMANDS § Finding them | The Templates header button is badged with the total, starters included | todo |
| `orch-three-views` | ORCHESTRATOR_COMMANDS § Finding them | The drawer has three top-level views: Active, Templates, Canvas | todo |
| `orch-active-view` | ORCHESTRATOR_COMMANDS § Finding them | Active lists one row per card carrying a workflow, stopped and waiting-on-you first; clicking a row closes the drawer and opens that card | todo |
| `orch-canvas-view` | ORCHESTRATOR_COMMANDS § Finding them | Canvas is the editor; with nothing open it shows an explanation instead of a blank panel | ct: test-ct/OrchestratorDrawer.canvas.spec.tsx |
