...
| `orch-node-command-run` | GUIDE § The Deck | A rule pointed at a command node runs a shell command from `agentFlow.commands` (`id`, `label`, `run`, optional `detail`) or free text; `{note}` is replaced with the rule's note, spliced in unquoted | e2e: a command node asks consent and act runs it |
