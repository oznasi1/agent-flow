...
| `orch-node-gate-card-block` | GUIDE § The Deck | The gate's Approve and Reject also render on the CARD's Workflow block; clicking them there answers the gate. PINNED DEFECT: the card's buttons are a silent no-op (the drawer's own gate node works) | e2e: Approve on the card's own workflow block answers the gate |
...
| `no-org-defaults` | README § Quick start | The extension ships no organization-specific defaults; `agentFlow.jira.baseUrl` and `agentFlow.jira.project` default to `""` in the manifest and the wizard collects them | unit: test/unit/config.test.ts |
