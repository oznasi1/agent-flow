...
| `orch-node-gate-card-block` | GUIDE § The Deck | The gate's Approve and Reject also render on the CARD's Workflow block; clicking them there answers the gate | e2e: Approve on the card's own workflow block answers the gate (PINNED — the card's buttons are a silent no-op) |
