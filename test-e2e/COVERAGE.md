...
| `orch-ceiling-stop` | ORCHESTRATOR_COMMANDS § The ceiling | A pass whose spends would take the lifetime total past `spendCeiling` performs none of them, writes the flow disarmed, warns naming the flow and the count, journals `armed` with `source: "ceiling"`; reaching the ceiling exactly is allowed | unit: test/unit/deckView.test.ts |
| `orch-ceiling-tally` | ORCHESTRATOR_COMMANDS § The ceiling | The count is derived from the journal's `fired` lines — launches and seeds as sessions, runs as commands; `errored` and `reset` change nothing — and rides `deck:flows` per flow | unit: test/unit/engine/orchestrator/journal.test.ts |
| `orch-ceiling-header` | ORCHESTRATOR_COMMANDS § The ceiling | The flow header shows what was spent and a ceiling field (blank means none, junk reverts); a template offers neither | ct: test/webview/OrchestratorDrawer.test.tsx |
