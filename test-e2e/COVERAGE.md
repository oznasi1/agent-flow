...
| `task-status-zero-targets` | CONNECTORS § 2. `TaskProvider` | Zero status targets is an info toast ("No status transitions available for {key}"), not an error | todo |
| `task-status-field-prompt` | CONNECTORS § 2. `TaskProvider` | A status target with a `fields` entry prompts for it and sends the value in `moveTo.values`; a `TaskWriteError` with `retryWith` re-prompts only the field it names | todo |
...
| `task-provenance-custom-label` | SETTINGS § table | `agentFlow.provenanceLabel` names the label that is stamped; `agentFlow.stampLabelOnWrite: false` skips it | todo |
...
| `task-add-to-my-sprint` | CONNECTORS § 3. The capability table | Add to my sprint records `addToSprint` and stamps the provenance label; absent on a task assigned to someone else | e2e: records addToSprint and stamps the provenance label |
| `task-add-to-sprint-name-only-identity` | CONNECTORS § 2. `TaskProvider` | A `me()` with `id: ""` refuses the sprint write with "Couldn't resolve your {label} account." and writes nothing | todo |
| `task-remove-from-sprint` | CONNECTORS § 3. The capability table | Remove from sprint records `removeFromSprint`, and offers Undo that puts the card back | e2e: records removeFromSprint |
...
| `deck-open-agents` | GUIDE § The Deck | The Deck shows every Claude Code session open on this machine, read from `~/.claude/sessions`; sessions attach to the card owning their directory, and a place with no tracked run gets a `local` card that disappears when its last session closes; `agentFlow.openAgents: false` removes them without reopening the panel | todo |
| `deck-local-card-inference` | GUIDE § The Deck | A local card reads its branch for a ticket key (`PROJ-5641-team-table` → `PROJ-5641`, marked `~inferred`) only when `agentFlow.jira.project` is set, and for its pull request | todo |
| `deck-track-it` | GUIDE § The Deck | ⋯ → Track it pins a local card to the runs store, after which it behaves like a task you took, Forget included | todo |
...
| `connector-me-identity` | CONNECTORS § 2. `TaskProvider` | `me()` returning `null` is treated as unknown (the header chip is skipped); `id: ""` drives the chip and is-this-mine affordances but refuses writes | todo |
