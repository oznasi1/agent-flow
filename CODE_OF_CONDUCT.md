...
maintainer. This project deliberately publishes no email address, so there are two private
routes:

- **[Open a private report](https://github.com/oznasi1/agent-flow/security/advisories/new)** —
  the repository's private advisory form. It is labelled for security, but it is the only
  channel here that is private between you and the maintainer, and conduct reports sent
  through it are welcome and treated the same way.
- **[Report to GitHub](https://github.com/contact/report-abuse)** — for anything that
  violates GitHub's own policies, or if you would rather not contact the maintainer
  directly. GitHub can act on accounts in ways a project maintainer cannot.
...
Be aware of the honest limit here: this is a small project with a single maintainer, who is
also the person you would be reporting to. If the report concerns that maintainer, use the
GitHub route.
