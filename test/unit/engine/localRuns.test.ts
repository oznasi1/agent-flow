...
describe("inferTicket", () => {
  ...
  it("refuses a branch that names another project", () => {
    ...
    expect(inferTicket("OTHER-12-x", "PROJ", BASE)).toBeNull();
  });
...
describe("localRunFor", () => {
  ...
  it("keys a workspace group off the workspace file itself, not the first root", () => {
    ...
    const run = localRunFor(
      { workspaceFile: "/ws/webapp+e2e.code-workspace", roots: ["/r/webapp", "/r/e2e_suite"], places: ["/r/webapp"] },
      [sess()], GIT, null, NOW,
    );
    expect(run.key).toBe(localKey("/ws/webapp+e2e.code-workspace"));
  });
  ...
  it("names a repo after its full path when its basename is empty", () => {
    ...
    const run = localRunFor(
      { workspaceFile: "/ws/x.code-workspace", roots: ["/r/webapp", "/"], places: ["/r/webapp"] },
      [sess()], GIT, null, NOW,
    );
...
describe("groupPlacesByWindow", () => {
  ...
  it("leaves a place alone when a workspace-kind window has a single root", () => {
    ...
    expect(groupPlacesByWindow(
      ["/r/webapp"],
      [ws("/ws/x.code-workspace", ["/r/webapp"])],
    )).toEqual([{ workspaceFile: null, roots: ["/r/webapp"], places: ["/r/webapp"] }]);
  });
