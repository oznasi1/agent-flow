...
import { CommandNode, Flow, FlowAction, FlowEdge, LaunchDest, PlaceNode, PlannedNode, SpendTally, emptyFlow, findNode, hasCeiling, isCommand, isPlace, isPlanned, isSettled, isSpendAction, overCeiling, spendTotal, stripHostStamps } from "./engine/orchestrator/model";
...
import { appendEvent, truncateOutput, findEdgeOutput, readJournal, spendTally, JournalEvent, JournalEventInput } from "./engine/orchestrator/journal";
...
export class DeckPanel {
  ...
  private postFlows(): void {
    ...
    this.post({
      ...
      commands: cfg.commands, branchCi, templates, spend,
    });
  }
  ...
  private reprobeForge(): void {
    ...
    this.forgeAccounts = [];
    // Resolved caps are probed alongside the gap and must be re-read with it: a
    // user who upgraded `atlassian-cli` mid-session is exactly who re-probing is
    // for, and a stale `projected` would keep the richer mode off until reload.
  ...
  private async submitReview(id: string, verb: ReviewVerb, body: string, fromDraft: boolean): Promise<void> {
    ...
    try {
      ...
      const text = fromDraft && cfg.stampLabelOnWrite && body.trim()
        ? `${body.trim()}\n\n${reviewProvenance(resolvedProvider(cfg.agentProvider))}`
        : body;
