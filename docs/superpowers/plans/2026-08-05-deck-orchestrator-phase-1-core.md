...
- **`npm run build` must succeed before every commit.** Typecheck and vitest are NOT sufficient: the webview is bundled by esbuild for a BROWSER target, so any module reachable from `src/webview/` must not import `fs`, `os`, `path` or `child_process` — even transitively, even if the importing code path never executes. A bundler resolves imports statically; Node does not. This gate was missing and a `conditions.ts` -> `status.ts` -> `fs` chain broke the build undetected through fourteen task reviews.
