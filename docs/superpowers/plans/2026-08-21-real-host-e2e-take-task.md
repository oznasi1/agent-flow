...
1. **Journey 2 — seeding**: ~~panel surface~~ **done** in `test-e2e/seed-panel.e2e.ts` against a pinned real Claude Code build (2.1.238, gunzipped from the gallery, cached per platform): the seeded tab opening pins the undocumented `claude-vscode.primaryEditor.open` contract. The prompt's text is NOT asserted there — signed out, the panel shows its login screen, and signing in from CI would call Anthropic's API (the spec's hard boundary); the identical seedText pipeline is already pinned by the terminal journey. The **terminal surface was done** earlier in `test-e2e/seed-terminal.e2e.ts` — plan file → new-window activation → identity match → terminal opened → prompt typed unsubmitted, asserted from real xterm DOM, with a `claude` shim so the developer's real CLI can never start. The **panel surface** (default) remains: it needs a pinned Claude Code build installed into the sandbox's `extensionsDir`, and its URI-handler rung must be neutralized first — `vscode://` on a developer machine routes to the REAL installed editor, the same escape class as `open -a`.
...
3. ~~**Journey 4 — worktree mode**~~ — **done** in `test-e2e/worktree-take.e2e.ts`: git-registered worktree, per-task branch, brief in the worktree and not the checkout. ~~**Journey 5**~~ — **done** in `test-e2e/sign-in.e2e.ts`, with ZERO product seams: the gate's own button drives the real InputBox flow, `fixture.invalid` fails the post-auth fetch deterministically, and sign-out re-gates after a refresh (signOut does not re-post state — a real UX gap, pinned not fixed).
