...
- **`IDLE_LIKE`** does **not**. A session waiting on your approval is not idle, and "Agent idle over N minutes" firing on it would auto-nudge past a modal dialog. That exclusion is deliberate and correct — and it is not free. Two **released** orchestrator conditions change behaviour as a direct consequence: `agent-idle-over` (`conditions.ts:105`) gates on `isIdleLike`, so a stale transcript with one of the six gated tool names pending used to read `stalled` (idle-like, so the rule fired) and now reads `blocked` (not idle-like, so the rule never fires on it). `agent-ended-turn` (`conditions.ts:104`) reads `mostActive`'s reduction, and because `blocked` ranks above `needs-you`, a place holding one session that ended its turn and one frozen at a gated tool now reduces to `blocked` instead of `needs-you`, so that condition stops firing too. Neither change was a design goal — both are named in the changelog so a user with either flow armed knows what changed and why.
