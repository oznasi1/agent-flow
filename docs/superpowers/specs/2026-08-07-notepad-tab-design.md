...
- **Status reuses the cheap live-session idiom, not the retire sweep.** `readRuns` + `groupByPlace(readOpenSessions(...))` is already how `describeActiveTasks` answers "is an agent attached to this run", and it costs two directory reads. `retireVerdict` answers a different, more expensive question (should this record be deleted) and needs inputs the Tasks panel doesn't build. The Notepad badge is a read-only projection of the cheap signals onto the note's `lastRunKey` — it never writes to the run store and never retires anything; that stays exclusively Deck's job.
...
- `src/webview/DeckApp.tsx` (~line 270) gets a `"notepad"` branch in the untracked-key label, beside the existing `local`/`explore` cases. Note the existing `explore` flag there is **key-prefix** based (`r.run.key.startsWith("explore-")`), not `runKind` — the notepad branch uses `runKind(r.run) === "notepad"`, which is exact, rather than adding a third prefix test. A notepad run has `url: ""`, so `isTicketRun` is already false and it correctly falls into that untracked branch with no other change.
...
- Unit test for the pure status-derivation function across all four cases: no record, `finishedAt` stamped, a repo path in `livePlaces`, and neither.
