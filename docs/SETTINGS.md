...
| `agentFlow.commandConsent` | `flow` | How a workflow asks before running a **shell command**. `flow` asks once per workflow and then runs every command it has, and every command added later, unattended — what every existing workflow does. `command` keys the approval to the *resolved command text*: each new text asks, and the ask offers **Run once**, **Run the next 5**, **Always for this command**, or **Disarm**. `agentFlow.neverAutoRun` outranks both. See [ORCHESTRATOR_COMMANDS.md](ORCHESTRATOR_COMMANDS.md#consent-per-command). |
